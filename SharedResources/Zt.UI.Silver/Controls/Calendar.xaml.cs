...
using Zt.UI.Silver.EventHandler;
using Zt.UI.Silver.Global;
...
namespace Zt.UI.Silver
{
    public sealed partial class Calendar : UserControl
    {
        ...
        #region Property
        public Global.CalendarMode CalendarMode
        {
            get { return (Global.CalendarMode)GetValue(CalendarModeProperty); }
            set { SetValue(CalendarModeProperty, value); }
        }

        public static readonly DependencyProperty CalendarModeProperty =
            DependencyProperty.Register(" CalendarMode", typeof(Global.CalendarMode), typeof(Calendar), new PropertyMetadata(OnCalendarModeChanged));
        ...
        private void Calendar_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            if (CalendarMode == Global.CalendarMode.Date)
            {
                ChangePanel(DayMonthYear.Day);
            }
            else if (CalendarMode == Global.CalendarMode.YearMonth)
            {
        ...
        private void RdbMonth_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (CalendarMode == Global.CalendarMode.Date)
                ChangePanel(DayMonthYear.Day);
            if (CalendarMode == Global.CalendarMode.YearMonth)
                RaiseSelectedEvent();
        }
        ...
        private void RdbYear_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (CalendarMode != Global.CalendarMode.Year)
                ChangePanel(DayMonthYear.Month);
            if (CalendarMode == Global.CalendarMode.Year)
                RaiseSelectedEvent();
        }
